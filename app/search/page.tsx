...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  return (
    <main className="py-10">
      <section className="mx-auto max-w-7xl px-4"><p className="text-gold">Archive search</p><h1 className="font-serif text-5xl">{q ? `Results for “${q}”` : 'Search the archive'}</h1><div className="mt-6"><SearchBox compact /></div>{results.usedFallback && <p className="mt-4 rounded-2xl border border-gold/25 bg-gold/10 p-4 text-vellum/75">No exact local record yet. Showing curated starter records; future versions can query open public APIs after local search.</p>}</section>
